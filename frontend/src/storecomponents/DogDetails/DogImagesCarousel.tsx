...
interface DogImagesCarouselProps {
    images: DogImage[];
    dogName: string;
}

const DogImagesCarousel: React.FC<DogImagesCarouselProps> = ({
    images,
    dogName,
}) => {
    const slides = useMemo<ReactImageGalleryItem[]>(() => {
        const list = images.length
            ? images
            : [{ id: 'def', url: 'https://placedog.net/600/400?random', altText: 'No image' }];
        return list.map(img => ({
            original: img.url,
            thumbnail: img.url,
            originalAlt: img.altText ?? dogName,
            thumbnailAlt: img.altText ?? `${dogName} thumb`,
        }));
    }, [images, dogName]);

    return (
        <div className="dog-images-carousel">
            <ImageGallery
                items={slides}
...
export default React.memo(DogImagesCarousel);